...
// Компонент тега для упражнений
const ExerciseTag = React.memo(({ label }: { label: string }) => {
  const getIcon = () => {
    if (label.toLowerCase() === "cardio") {
      return <SvgXml xml={cardioIcon} width={16} height={16} />;
    } else if (
      label.toLowerCase() === "strenght" ||
      label.toLowerCase() === "strength"
    ) {
      return <SvgXml xml={strengthIcon} width={16} height={16} />;
    }
    return null;
  };
...
const ExerciseCard = React.memo(({ exercise }: { exercise: Exercise }) => (
  <Pressable
    ...
    >
      ...
      <View style={styles.exerciseTags}>
        {exercise.body_parts && exercise.body_parts.length > 0 && (
          <ExerciseTag label={exercise.body_parts[0]} />
        )}
        {exercise.dif_level && (
          <ExerciseTag
            label={
              exercise.dif_level.charAt(0).toUpperCase() +
              exercise.dif_level.slice(1)
            }
          />
        )}
      </View>
...
export default function AvailableExercisesScreen() {
  ...
  const fetchExercises = useCallback(async (params: FilterParams = {}) => {
    try {
      setLoading(true);
  ...
  const applyFilters = useCallback(() => {
    ...
    fetchExercises(newFilterParams);
  }, [selectedFilters, fetchExercises]);
  ...
  return (
    ...
    >
      ...
      {/* Title */}
      <Text style={styles.pageTitle}>Available exercises</Text>
      ...
      {loading ? (
        ...
        <View style={styles.errorContainer}>
          ...
          <TouchableOpacity
            ...
            onPress={() => fetchExercises(filterParams)}
          >
